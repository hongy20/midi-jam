...
- **Editor Experience**: No changes to `eslint.config.mjs` rules are required. By _not_ using `eslint-plugin-prettier`, we avoid showing formatting violations as linting errors in the IDE, adhering to the "Option B" preference.
...
2. **CI/CD pipeline**:
   ...
   - Fails if _any_ file is unformatted or contains linting errors.
