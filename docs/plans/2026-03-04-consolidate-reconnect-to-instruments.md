...
## Problem
The project has a concept of a `/reconnect` route for mid-game device disconnections, but it lacks a dedicated page and the redirection logic doesn't preserve game state reliably. The goal is to use the existing `/gear` page for reconnection and ensure the game state is saved when a device is lost.
...
### 5. Cleanup Tests
- **`src/app/play/page.test.tsx`** & **`src/app/page.test.tsx`**:
...
1. **Manual Test: Mid-game disconnect**:
   ...
   - Verify redirect to `/gear?from=game`.
   - Reconnect/select a device and click "Resume Game".
   - Verify return to `/pause` with the correct score and time.
2. **Manual Test: Normal flow**:
   - Verify that navigating to `/gear` from the main menu still works correctly and continues to `/collection`.
3. **Automated Tests**:
