...
1.  **Long note split bug**: In `buildSegmentGroups`, the gap check (`span.startTimeMs - currentStartMs >= thresholdMs`) compares a new note's start time against the cluster's _first_ note's start time, not the cluster's end time. This incorrectly splits long notes (e.g., long sustained chords) when a short note appears later but still within the long note's duration.
