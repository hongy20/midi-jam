"use client";

import type { Accuracy } from "@/hooks/use-score-engine";

interface ScoreHudProps {
  score: number;
  combo: number;
  lastAccuracy: { type: Accuracy; id: number } | null;
  highScore: number;
  bestCombo: number;
}

/**
 * Animated accuracy feedback that only renders when a new result arrives.
 */
const AccuracyFeedback = ({
  accuracy,
}: {
  accuracy: { type: Accuracy; id: number } | null;
}) => {
  if (!accuracy || !accuracy.type) return <div className="h-16" />;

  return (
    <div className="h-16 flex items-center justify-center">
      <div
        key={accuracy.id}
        className="text-4xl font-black tracking-tighter italic text-white animate-accuracy-pop"
      >
        {accuracy.type}!
      </div>
    </div>
  );
};

/**
 * Static-ish stats like high score and best combo.
 */
const PersistentStats = ({
  highScore,
  bestCombo,
}: {
  highScore: number;
  bestCombo: number;
}) => {
  return (
    <div className="flex items-center gap-4 mt-1">
      <div className="flex flex-col items-center bg-black/40 backdrop-blur-sm border border-white/10 px-3 py-1 rounded-xl text-white shadow-lg">
        <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">
          High Score
        </span>
        <span className="text-sm font-black tabular-nums">
          {highScore.toFixed(1)}
        </span>
      </div>

      {bestCombo > 0 && (
        <div className="flex flex-col items-center bg-black/40 backdrop-blur-sm border border-white/10 px-3 py-1 rounded-xl text-white shadow-lg">
          <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">
            Best Combo
          </span>
          <span className="text-sm font-black tabular-nums">{bestCombo}x</span>
        </div>
      )}
    </div>
  );
};

/**
 * High-performance Score HUD.
 * Separates static stats from high-frequency score/combo/accuracy updates.
 */
export function ScoreHud({
  score,
  combo,
  lastAccuracy,
  highScore,
  bestCombo,
}: ScoreHudProps) {
  return (
    <div className="absolute top-12 left-1/2 -translate-x-1/2 flex flex-col items-center pointer-events-none select-none z-40">
      {/* Dynamic Accuracy layer */}
      <AccuracyFeedback accuracy={lastAccuracy} />

      {/* Dynamic Score & Combo */}
      <div className="flex flex-col items-center gap-1 mt-4">
        <div className="text-6xl font-black text-white drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)] tabular-nums">
          {score.toFixed(1)}
        </div>

        <div className="flex flex-col items-center gap-2">
          {combo >= 2 && (
            <div className="bg-white/90 backdrop-blur-md px-6 py-1 rounded-full text-slate-900 font-black text-xl shadow-xl animate-in zoom-in duration-300">
              {combo}x COMBO
            </div>
          )}

          {/* Static Stats layer */}
          <PersistentStats highScore={highScore} bestCombo={bestCombo} />
        </div>
      </div>
    </div>
  );
}
